...
function FreehandCircleDrawer() {
  ...
  const calculatePerfectness = () => {
    ...
    if (lines.length > 0) {
      ...
      console.log(
        `standard deviationAngle: ${standardDeviationAngle.toFixed(2)}`
      );
      console.log(
        `Radius: ${averageDistance.toFixed(2)}`
      );
      ...
      perfectnessScore =
        1 / (1 + Math.exp(-(perfectnessScore * 1 + radiusBias)));

        let finalScore = perfectnessScore * 10;
        // Handling the point only case
